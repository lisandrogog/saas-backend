...
export class SalesService {
  ...
  async getOrderById(tenantId: string, businessUnitId: string, id: string) {
    const order = await this.prisma.sales_order.findUniqueOrThrow({
      where: {
        id,
        tenant_id: tenantId,
    ...
    const productIds = order.sales_order_item.map((item) => item.product_id);
    ...
    const mappedOrder = {
      ...order,
      sales_order_item: order.sales_order_item.map((item) => ({
        ...item,
        product: {
          ...item.product,
